...
merge 1:1 year using "./Work/USA_scenario_5_annual_results.dta" // merge inv cost results
gen elas_cap_priv_noprofit = 1 - elas_comp // create single capital elasticity
keep year elas_cap*

merge 1:1 year using "./Work/USA_scenario_6_annual_results.dta" // merge user cost results
gen elas_cap_priv_deprcost = 1 - elas_comp // create single capital elasticity
keep year elas_cap*
...
twoway rarea elas_cap_deprcost elas_cap_noprofit year, color(gray) fintensity(10) lwidth(0) ///
	|| rarea elas_cap_priv_deprcost elas_cap_priv_noprofit year, color(gray) fintensity(40) lwidth(0) ///
	...
	|| line elas_cap_priv_noprofit year, lcolor(black)  ///
	|| line elas_cap_priv_deprcost year,  lcolor(black) ///	
	...
	text(.33 2020 "1/3") ///
	legend(ring(0) pos(2) cols(1) region(lcolor(white)) order(- "Private business only:" 2 "Range" 5 "Bounds" - " " "All (Priv. bus. plus housing and gov.):" 1 "Range" 3 "Bounds"))
