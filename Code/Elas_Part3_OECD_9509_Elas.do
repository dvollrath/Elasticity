...
file open f_result using "./Work/OECD_STAN4_9509_elasticity.csv", write replace
file write f_result "cou, year, assump, elas_labor, elas_capital, va, emp" _n

local fileList : dir "./CSV" files "OECD_???????_io_2015Ed.csv" // get list of all IO tables

//local fileList = "OECD_FRA1998_io_2015Ed.csv"
...
foreach file of local fileList { // for each of the IO files
	...
	use "./Work/OECD_STAN4_9509_all.dta", clear // load up STAN database
	keep if cou=="`country'"
	keep if year==`year'
	count
...
capture file close f_result
insheet using "./Work/OECD_STAN4_9509_elasticity.csv", clear
gen series = 9509
save "./Work/OECD_STAN4_9509_elasticity.dta", replace
