...
foreach i in 1 2 3 4 { // for each baseline scenario file
	di "Scenario `i'"
	...
	forvalues y = 1997(1)2018 { // for each year for which the TR table exists
		...
		preserve
			...
			qui do "./Code/Elas_Part3_CalcImport.do" // Call script to calculate elasticities
		restore
	...
	qui save "./Work/USA_scenario_`scenario'_import_results.dta", replace // save individual results file
}
	
clear
qui capture rm "./Work/USA_scenario_import_results_all.dta"
qui save "./Work/USA_scenario_import_results_all.dta", emptyok replace
	
foreach i in 1 2 3 4 { // for each scenario file
	di "File `i'"
	qui append using "./Work/USA_scenario_`i'_import_results.dta"
}
qui save "./Work/USA_scenario_import_results_all.dta", replace		
